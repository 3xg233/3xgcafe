# 3xgcafe Monitor 维护手册

> 3xgcafe 系列工具之一 · 程序集名 `3xgcafe-Monitor`（原名 MiniMonitor）
...
3xgcafe Monitor 是一个常驻桌面角落的半透明悬浮小窗，黑底白线风格，一眼看清电脑状态。监控**十项**指标：
...
```
MiniMonitor/
├── MiniMonitor.csproj       # 项目文件：目标框架、单文件发布、运行时优化、NuGet 依赖（Version 2.0.0，产物名 3xgcafe-Monitor）
...
- 框架依赖版：双击 `dist/3xgcafe-Monitor.exe`（需 .NET 9 桌面运行时）。
- 便携版：双击 `dist-portable/3xgcafe-Monitor.exe`（无需运行时）。
- 操作：
...
| 数据 | 路径 |
|------|------|
| 设置 | `%APPDATA%\3xgcafe\Monitor\settings.json` |
| 崩溃日志 | `%APPDATA%\3xgcafe\Monitor\crash.log`（异常时写入） |
...
### 8.1 多实例互斥
应用启动时创建命名 Mutex `Local\3xgcafe-Monitor_Singleton`；若已存在则直接退出，保证只有一个悬浮窗。

### 8.2 开机自启（注册表方式）
在 **当前用户** 注册表键
`HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`
下写入值名 `3xgcafe-Monitor`，数据为本进程 exe 的完整路径（带引号）。取消勾选时删除该值。
**每次读写都会顺带删除旧版本的值名 `MiniMonitor`**，避免新旧版本同时自启。
> 注意：本系列其他工具同样使用注册表自启，但值名各不相同（`3xgcafe-Spotlight` / `3xgcafe-Wallpaper` / `3xgcafe-Guard`），维护时不要混淆。
...
- **通道名**：命名管道 `3xgcafe-Monitor`；协议为「一行 JSON 请求 → 一行 JSON 响应」，**一请求一连接、用完即断**（无连接时几乎零开销）。
- **实现**：`Ipc/ToolIpcServer.cs`（通用服务端）+ `App.xaml.cs` 的 `HandleIpc`。
- **指令**：`status` / `show` / `toggle_topmost` / `quit`。
- **`status` 的 `data` 字段**：`cpu` / `mem` / `temp` / `gpu0` / `gpu1` / `disk0` / `disk1` / `battery` / `topmost` / `autostart`。
- **响应格式**：`{"ok":bool,"state":"running|paused|locked|stopping|error","detail":"...","ver":1,"data":{...}}`。
- **消费方**：3xgcafeConsole 统一管理面板（工具清单见 `%APPDATA%\3xgcafeConsole\tools.json`）。
- ⚠️ **维护提示**：`HandleIpc` 依赖 `MainWindow` 的公开成员（`Monitor`、`IsTopmost`、`IsAutoStart`、`ToggleTopmost`）。改动窗口侧命名时，两处必须同步，否则面板会取不到状态。
...
| 多实例无法启动 | 已有实例持有 `Local\3xgcafe-Monitor_Singleton`，关闭旧实例即可。 |
| 开机未自启 | 检查注册表 `HKCU\...\Run\3xgcafe-Monitor` 值是否存在且路径有效。 |
...
| 项目 | 取值 |
|------|------|
| 显示名（窗口标题 / 托盘 / 文档） | `3xgcafe Monitor` |
| 程序集 / exe 名 | `3xgcafe-Monitor`（`3xgcafe-Monitor.exe`） |
| 注册表自启值名 | `3xgcafe-Monitor`（旧 `MiniMonitor` 自动清理） |
| 单实例互斥名 | `Local\3xgcafe-Monitor_Singleton` |
| 数据目录 | `%APPDATA%\3xgcafe\Monitor\` |
| IPC 通道名 | `3xgcafe-Monitor`（旧 `3xgcafe-MiniMonitor`） |
| 工程目录 / 命名空间 | 保持 `MiniMonitor`（仅内部标识，不改） |

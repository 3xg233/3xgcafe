# 3xgcafe Spotlight 维护手册

> 3xgcafe 系列工具之一 · 程序集名 `3xgcafe-Spotlight`（原名 SpotlightLauncher）
...
3xgcafe Spotlight 是按下全局热键即呼出的聚焦式搜索启动器，UI 复刻《终末地》游戏语言（黑灰底 + 荧光黄绿 RGB(250,252,82) + 等高线 + 工业终端标题栏）。
...
```
SpotlightLauncher/
├── SpotlightLauncher.csproj   # 项目文件：目标框架、单文件发布、运行时优化、NuGet 依赖（Version 2.0.0，产物名 3xgcafe-Spotlight，含 ApplicationIcon）
...
- 框架依赖版：双击 `dist/3xgcafe-Spotlight.exe`。
- 便携版：双击 `dist-portable/3xgcafe-Spotlight.exe`。
...
| 数据 | 路径 | 说明 |
|------|------|------|
| 剪贴板历史 | `%APPDATA%\3xgcafe\Spotlight\clipboard\history.json` + `images/{id}.png` | 文本存 JSON；图片存 PNG |
| 使用频率 | `%APPDATA%\3xgcafe\Spotlight\freq.json` | 常用应用加权排序 |
| 偏好设置 | `%APPDATA%\3xgcafe\Spotlight\autostart.json` | `{"AutoStart": bool, "KeepOnDeactivate": bool}` 双键 |
| 窗口位置 | `%APPDATA%\3xgcafe\Spotlight\window.json` | `{"Left": double, "Top": double}` |
| 自定义扫描路径 | `%APPDATA%\3xgcafe\Spotlight\scanpaths.json` | 字符串数组；兼容旧对象格式读取 |
| 崩溃日志 | `%APPDATA%\3xgcafe\Spotlight\crash.log` | AppDomain / Dispatcher 未处理异常 |

> ⚠️ **自启状态的真源是注册表**（`HKCU\...\Run\3xgcafe-Spotlight`）；`autostart.json` 只记录用户的**偏好**，用于避免"取消勾选后被静默恢复"。两者需成对维护。
...
### 8.1 多实例互斥
启动时创建命名 Mutex `Local\3xgcafe-Spotlight_Singleton`；已存在则提示并退出，保证单一实例。
...
### 8.3 开机自启（注册表 + 偏好双写）
- 真源：`HKCU\Software\Microsoft\Windows\CurrentVersion\Run` 下的值名 `3xgcafe-Spotlight`，数据为 exe 完整路径。**每次读写都会顺带删除旧版本的值名 `SpotlightLauncher`**，避免新旧版本同时自启。
...
- ⚠️ **维护提示**：`IndexReady` / `IndexedCount` 取自 `_indexer`；`ClipboardReady` 取 `_clipReady` 标志，`ClipboardCount` 用 `_clipStore.Search(null, ClipboardStore.MaxTotalItems).Count` 计算 —— 因为 v1.4 死代码清理时**删除了 `ClipboardStore.Count` / `IsReady`**，不要再试图调用它们。
...
| 多实例无法启动 | 已有实例持有 `Local\3xgcafe-Spotlight_Singleton`，关闭旧实例即可。 |
...
| 项目 | 取值 |
|------|------|
| 显示名（窗口标题 / 托盘 / 文档） | `3xgcafe Spotlight` |
| 程序集 / exe 名 | `3xgcafe-Spotlight`（`3xgcafe-Spotlight.exe`） |
| 注册表自启值名 | `3xgcafe-Spotlight`（旧 `SpotlightLauncher` 自动清理） |
| 单实例互斥名 | `Local\3xgcafe-Spotlight_Singleton` |
| 数据目录 | `%APPDATA%\3xgcafe\Spotlight\`（clipboard / freq.json / autostart.json / window.json / scanpaths.json / crash.log） |
| IPC 通道名 | `3xgcafe-Spotlight`（旧 `3xgcafe-SpotlightLauncher`） |
| 工程目录 / 命名空间 | 保持 `SpotlightLauncher`（仅内部标识，不改） |

> 这套隔离确保新版与旧版能在同一台机器共存而不互相干扰：进程名、互斥量、自启项、数据目录、IPC 通道均不复用。注意**数据目录变更后，旧版的历史记录（剪贴板、使用频率、自定义扫描路径）不会自动迁移**，需要的话从 `%APPDATA%\SpotlightLauncher\` 手动复制到新目录。
